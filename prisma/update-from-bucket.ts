...
async function main() {
  console.log("🚀 Updating thumbnailUrl from existing bucket files...\n");

  try {
    // 1. Storage에서 파일 목록 가져오기
    console.log("🔍 Fetching files from contents_thumbnail bucket...");
    ...
    console.log(`📋 First 5 files: ${imageFiles.slice(0, 5).map((f) => f.name).join(", ")}\n`);
    ...
    for (const file of imageFiles) {
      ...
      // 이미 Supabase URL이 설정되어 있으면 스킵
      if (content.thumbnailUrl?.includes("supabase.co/storage")) {
        alreadySetCount++;
        continue;
      }
      ...
      console.log(`   Old URL: ${content.thumbnailUrl?.substring(0, 50)}...`);
    ...
    console.log(`   ⏭️  Already using Supabase: ${alreadySetCount}`);
    console.log(`   ⚠️  File not matched to content: ${notFoundCount}`);
